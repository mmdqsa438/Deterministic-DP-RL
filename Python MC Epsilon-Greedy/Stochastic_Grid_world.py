...
class GridWorld :
    ...
    def __init__(self, rows, cols, start_state):
        ...
        self.cols:int = cols
        self.rows:int = rows
        self.__invalid_action_reward: float = -10000
    ...
    def transition(self, action:str ) -> float :
        ...
        if action not in self.get_actions(self.current_state):
            return self.__invalid_action_reward
...
#-------------------------------
def standard_sticky_GW(cost: float = 0.0 ):
    r = 3
    c = 4
    start_state = (2,0)
...
def print_q_func(q: dict):
    
    print("-------------------------------------------")
    for key in q:
        print("| state:",key[0],"| action:",key[1],"| Qvalue:",q[key],"|")
        print("-------------------------------------------")
