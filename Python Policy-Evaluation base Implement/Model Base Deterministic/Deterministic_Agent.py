...
class Agent:
    ...
    def __init__(self) -> None :
        ...
        self.policy:dict = {}                      # Deterministic Agent_class policy    : { (i,j):action }
        self.value_state_function:dict = {}        # State-value function V(s)  : { (i,j):value  }
        
        self.current_state:tuple                   # [!] it maybe creat ambiguity if forgot to update
        ...
        self.model_states:list = []                # modeliment Visited States  : [ (i,j), (i',j') ]
        self.model_actions:list = []               # modeliment Done Actions    : [ "act1", "act2" ]
        self.model_rewards:dict = {}               # modeliment Gained rewards  : { (i,j):reward }
    ...
    def policy_probability_disribution( self,action:str, state:tuple) -> bool :
        return action == self.policy.get(state,"$@#)RFW")
    
    
    # V(s) Bellman equation State-value function.
    #----------------------------------------
    def state_Value( self, current_state:tuple, probability_func )-> float:
        
        value = 0
        for act in self.model_actions :
            
            pi_a_s = self.policy_probability_disribution( act, current_state )
            
            for next_state in self.model_states :
                value += pi_a_s * probability_func(next_state, current_state, act ) * ( self.model_rewards.get(next_state,0) + self.__GAMMA * self.value_state_function.get(next_state,0.0))
    ...
    #----------------------------------------
    def Evaluate_policy(self, probability_func )-> None:
        
        for it in range(self.__MAX_ITR) :
            
            max_change = 0
            for st in self.model_states :
                
                old_value = self.value_state_function.get(st)
                self.value_state_function[st] = self.state_Value(st,probability_func)
    ...
    def action_Value(self, state, action, probability_func)-> float:
        ...
        for _ in range(self.__MAX_ITR):
            for next_st in self.model_states:
                value += probability_func(next_st, state, action) * (self.model_rewards.get(next_st, 0.0) + self.__GAMMA * self.value_state_function.get(next_st, 0.0) )
    ...
    def improve_policy(self,env) -> bool:
        ...
        for st in env.actions :
            ...
            self.policy[st] = self.best_action(st,env.probability)
    ...
    # Finding Best action.
    #----------------------------------
    def best_action(self, state,probability_func)-> str:
        ...
        for act in self.model_actions :
            value = self.action_Value(state,act,probability_func)
